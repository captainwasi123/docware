...
$(document).ready(function(){
	  ...
	  $("#voyage_select").change(function(){
	   ...
	   $("#fShippingline").change(function(){
	  		var value = this.value;
	  		var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
			    if (this.readyState == 4 && this.status == 200) {
				    document.getElementById("ship_company").value = xhttp.responseText;
			    }else{
				    document.getElementById("ship_company").value = "..";
			    }
			};
			xhttp.open("GET", host+"/mbl/masterBL/response/getShipCompany/"+value, true);
			xhttp.send();
	  });
});